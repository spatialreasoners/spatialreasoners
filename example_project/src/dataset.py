...
def generate_spiral_data(n_points=1000, noise=0.5, num_loops=4):
    """Generate spiral dataset points."""
    r = np.sqrt(np.random.rand(n_points)) # sqrt for more uniform area coverage
...
class SpiralDataset(sr.dataset.IndexedDataset[SpiralDatasetCfg]):
    ...
    def num_classes(self) -> int:
        """Number of classes in the dataset. Spiral data is unconditional, so we have 0 classes"""
        return 0
