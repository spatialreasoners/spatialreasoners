...
def enable_beartype_checking():
    ...
    try:
        ...
        # Configure beartype and jaxtyping for the spatialreasoners package
...
def _get_embedded_config_dir() -> Path:
    ...
    1. Development: Uses the config/ directory parallel to spatialreasoners/
    ...
    try:
        ...
        with importlib.resources.path("spatialreasoners.configs", "__init__.py") as config_init:
    ...
    # Development fallback: look for config directory parallel to spatialreasoners
    current_file = Path(__file__)
    sr_dir = current_file.parent
...
def run_training(
    ...
    **trainer_kwargs
    ...
    - Loading configuration (embedded or custom+embedded)
    - Creating all necessary components
    - Setting up the trainer with proper callbacks and logging
    - Running the training loop
    
    Args:
        config_path: Path to config directory (uses only embedded configs if None)
    ...
    if typed_config.wandb.activated:
        ...
        if wandb.run is not None:
            wandb.run.log_code("spatialreasoners")
    else:
        logger = LocalLogger()
